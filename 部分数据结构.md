...
int findfa(int x) {
    return heap[x].fa == x ? x : heap[x].fa = findfa(heap[x].fa);
}
...
int merge(int x, int y) {
    ...
    if (dis[heap[x].val] < dis[heap[y].val]) {
        swap(x, y);
    }
    ...
    heap[x].fa = x;
    if (heap[x].ls) heap[heap[x].ls].fa = x;
    if (heap[x].rs) heap[heap[x].rs].fa = x;
    return x;
}

int pop(int x) {
    heap[heap[x].ls].fa = heap[x].ls;
    heap[heap[x].rs].fa = heap[x].rs;
    return heap[x].fa = merge(heap[x].ls, heap[x].rs);
}
```
